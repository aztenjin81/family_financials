...
export function MoneyV({ value, size = 'sm', serif = false, hidden = false, cents = false, sign = false, className = '', forceSign = false }) {
...
export function Avatar({ who, size = 32, style = {} }) {
  const m = FAMILY[who];
  if (!m) return null;
  return (
    <span
...
export function Sparkline({ data, color = 'var(--ink)', fill = 'rgba(21,20,15,0.06)', height = 60 }) {
  const w = 100;
  const h = height;
...
export function Ring({ value, total, size = 96, stroke = 10, color = 'var(--ink)', track = 'var(--paper-2)', children }) {
